...
const char* GITHUB_TOKEN = "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";
const char* versionUrl = "https://raw.githubusercontent.com/GuilhermeMendina/OTA-CI/main/version.txt";
const char* firmwareUrl = "https://raw.github.com/repos/GuilhermeMendina/OTA-CI/main/ota.bin";
...
void setup() {
  ...
  pinMode(pinToFactoryReset, INPUT_PULLUP);
  Serial.begin(115200);
...
bool ConnectNimBLE() {
  ...
  if (pChr->canWrite()) {
    ...
    if ((100 <= releaseNumber) && (releaseNumber < 200)) {
      if (TOKEN.empty() || TOKEN_2.empty() || TIMESTAMP.empty())
        return false;
    ...
    if ((200 <= releaseNumber) && (releaseNumber < 300)) {
      if (TOKEN.empty() || TOKEN_2.empty() || TIMESTAMP.empty() || ACTION.empty())
        return false;
...
void CallbackMQTT(char* topic, byte* payload, unsigned int length) {
  ...
  if (arguments[0].compare("token") == 0) {
    ...
    if ((100 <= releaseNumber) && (releaseNumber < 200)) {
      if (arguments[3].empty() || arguments[4].empty() || arguments[5].empty())
        return;
    ...
    if ((200 <= releaseNumber) && (releaseNumber < 300)) {
      if (arguments[3].empty() || arguments[4].empty() || arguments[5].empty() || arguments[6].empty())
        return;
...
int base64_decode(char* output, const char* input, int inputLen) {
  ...
  while (inputLen-- && (input[in] != '=') && isBase64(input[in])) {
...
void checkAndUpdateOTA() {
  ...
  //client.setInsecure();  // necessário para GitHub SSL
  HTTPClient https;

  Serial.println("[OTA] Baixando firmware via GitHub privado...");
  https.begin(client, firmwareUrl);
  https.addHeader("Authorization", String("token ") + GITHUB_TOKEN);
  int httpCode = https.GET();

  if (httpCode == HTTP_CODE_OK) {
    String payload = https.getString();

    int start = payload.indexOf("\"content\":\"") + 10;
    String contentBase64 = payload.substring(start, payload.indexOf("\",", start));
    contentBase64.replace("\\n", "");
    contentBase64.replace("\n", "");

    int firmwareLen = (contentBase64.length() * 3) / 4;
    uint8_t firmwareBin[firmwareLen];
    int actualLen = base64_decode((char*)firmwareBin, contentBase64.c_str(), contentBase64.length());

    Serial.printf("[OTA] Tamanho decodificado: %d bytes\n", actualLen);

    if (Update.begin(actualLen)) {
      size_t written = Update.write(firmwareBin, actualLen);
      if (written == actualLen) {
        if (Update.end()) {
          if (Update.isFinished()) {
            ...
            Serial.println("[OTA] Update incompleto.");
          }
        } else {
          Serial.printf("[OTA] Erro: %s\n", Update.errorString());
        }
      } else {
        Serial.println("[OTA] Erro na escrita.");
      }
    } else {
      Serial.println("[OTA] Falha ao iniciar Update.");
    }
...
void checkResetButton() {
  if (digitalRead(pinToFactoryReset) == LOW) {
    ...
    if (pressCount >= 4) {
      Serial.println("[RESET] Detecção de 4 cliques. Trocando partição de boot...");

      const esp_partition_t* running = esp_ota_get_running_partition();
      const esp_partition_t* next = esp_ota_get_next_update_partition(running);
      esp_ota_set_boot_partition(next);

      Serial.println("[RESET] Reiniciando para firmware anterior...");
      delay(1000);
      ESP.restart();
    }

    while (digitalRead(pinToFactoryReset) == LOW) delay(10);  // Debounce
  }
}
